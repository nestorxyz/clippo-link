...
export async function GET(request: Request) {
  ...
  if (access_token && provider_token) {
    ...
    const { data, error } = await retired-provider.auth.signInWithIdToken({
      provider: 'google',
      access_token,
      token: provider_token,
    });
    console.log('retired-provider Auth Data:', data);
    console.log('retired-provider Auth Error:', error);
