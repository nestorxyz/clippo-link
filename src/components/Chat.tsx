...
const Chat = ({
  ...
  useEffect(() => {
    ...
    const loadOrCreateChatSession = async () => {
      ...
      try {
        ...
        } = await retired-provider.from('chat_sessions').select('id').eq('user_id', session.user.id).order('created_at', {
          ascending: false
        }).limit(1).maybeSingle();
        ...
        if (existingSession) {
          ...
          } = await retired-provider.from('chat_messages').select('id, parts, role').eq('session_id', currentSessionId).order('created_at', {
            ascending: true
          });
          ...
          } = await retired-provider.from('chat_sessions').insert({
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await retired-provider.functions.invoke('gemini-chat', {
        body: {
          sessionId,
  ...
  const handleClearChat = async () => {
    ...
    try {
      const {
        error
      } = await retired-provider.from('chat_messages').delete().eq('session_id', sessionId);
