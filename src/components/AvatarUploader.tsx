...
export const AvatarUploader = ({
  ...
  async function downloadImage(path: string) {
    try {
      const { data, error } = await retired-provider.storage.from('avatars').download(path);
      if (error) {
        throw error;
      }
  ...
  const uploadAvatar = async (event: React.ChangeEvent<HTMLInputElement>) => {
    try {
      ...
      const { error: uploadError } = await retired-provider.storage.from('avatars').upload(filePath, file);
