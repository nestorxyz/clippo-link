  ...
  return <div className={cn("h-full w-full")}>
      <div className="flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          {categories.map(category => <Collapsible key={category.id} open={openCategories.includes(category.id)} onOpenChange={() => toggleCategory(category.id)}>
              ...
              <CollapsibleContent className="pl-6 space-y-1 py-1">
                {category.subCategories.map(sub => <div key={sub.id}>
                    ...
                    {!isCollapsed && sub.links.map(link => <div key={link.id} className="ml-6 pr-2 py-1.5 rounded-md hover:bg-secondary/50 group">
                        ...
                        {link.tags.length > 0 && <div className="mt-1.5 flex flex-wrap gap-1">
                            {link.tags.map((tag: Tag) => <Badge key={tag.id} variant={tag.color ? "default" : "secondary"} className="text-xs font-normal" style={tag.color ? {
