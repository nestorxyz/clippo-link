...
const SubCategoryManager = ({ session }: { session: Session | null }) => {
  const { data: categories = [], isLoading: isLoadingCategories } = useCategories(session);
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast.error('Failed to create sub-category', { description: error.message });
    }
  };
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="e.g. Project A" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl><Input placeholder="Briefly describe the sub-category" {...field} /></FormControl>
                  <FormMessage />
                </FormItem>
              )}
            />
                      ...
                      <li key={cat.id}>
                        ...
                        <ul className="pl-4 mt-1 space-y-1">
                          {cat.subCategories.map(sub => (
                            <li key={sub.id} className="p-2 bg-secondary rounded-md text-sm">{sub.name}</li>
                          ))}
                        </ul>
                      </li>
                    )
